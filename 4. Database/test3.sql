...
select ename, substr(hiredate,1,5) as "입사년월" -- 'yy/mm' 
from emp;
...
select trunc(sysdate - to_date('2019/01/01','yyyy/mm/dd')) as "디데이" 
from dual;
...
--26. 관리자 수를 나열하시오.
select count(mgr) as "관리자 수" from emp; -- null값 제외
...
select max(sal)-min(sal) as "급여 최고액 - 급여 최저액" from emp;
...
select deptno, count(*) as "사원수", round(avg(sal),2) as "평균 급여"
from emp
group by deptno;
